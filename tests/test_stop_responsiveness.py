...
def test_search_worker_granular_rust_stop():
    ...
    def side_effect(*args, **kwargs):
        callback = kwargs.get("results_callback")
        if callback:
            # 콜백 실행 -> signals.results_found 발생 -> worker.stop() 호출
            callback([("C:/dir1/f1.txt", [(1, "match", 0, 5)])])
        return mock_res

    with (
        patch("core.search_engine.HAS_RUST_ENGINE", True),
        patch("core.search_engine.search_directory_fast", side_effect=side_effect) as mock_search,
    ):
        # 1. 중간에 중단
