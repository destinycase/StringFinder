...
def test_initial_ui_state(search_tab_fixture):
    ...
    # ConfigManager has 4 default extensions
...
def test_preview_logic(qtbot, search_tab_fixture, tmp_path):
    """Verify preview logic for different file types and modes."""
    from utils.app_strings import AppStrings

    # 1. JSON file - Should show preview
    ...
    # Check if preview contains the text (it uses HTML)
    ...
    # 2. Text file - Should show preview
    ...
    # 3. Binary file - Should show error
    ...
    # 4. Excel Mode (even if file is text-like for some reason) - Should show error
...
def test_excel_location_in_normal_mode(qtbot, search_tab_fixture):
    """Verify that Excel file results show correct headers/positions in Normal mode."""
    from utils.app_strings import AppStrings

    # Mock 3-tuple results typical for Excel
    excel_path = "D:/test/data.xlsx"
    matches = [(0, "Sheet1!A1", "Match 1"), (0, "Sheet1!B5", "Match 2")]

    # 1. Normal Mode (Special Search OFF)
    search_tab_fixture.special_search_combo.setCurrentText(AppStrings.SPECIAL_SEARCH_OFF)

    # Call set_matches directly on the model
    # Normally this is called by _show_matches_from_view
    search_tab_fixture.match_model.set_matches(
        excel_path, matches, search_text="Match", search_mode=AppStrings.SPECIAL_SEARCH_OFF
    )

    # Verify headers are Excel headers
    assert search_tab_fixture.match_model._headers == [AppStrings.HEADER_EXCEL_POSITION, AppStrings.HEADER_EXCEL_VALUE]

    # Verify data is 2-tuple (Location, Value)
    assert search_tab_fixture.match_model.rowCount() == 2
    assert search_tab_fixture.match_model.data(search_tab_fixture.match_model.index(0, 0), Qt.EditRole) == "Sheet1!A1"
    assert search_tab_fixture.match_model.data(search_tab_fixture.match_model.index(0, 1), Qt.EditRole) == "Match 1"
