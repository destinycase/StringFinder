...
def test_scan_worker_smart_scan_basic(qtbot, signal_receiver):
    ...
    with patch(f"{ENGINE_MODULE}.HAS_RUST_ENGINE", True):
        with patch(f"{ENGINE_MODULE}.find_files_with_keyword_fast", return_value=mock_files) as mock_find:
            ...
            # Connect signals
            worker.scan_finished.connect(signal_receiver.on_scan_finished)
            worker.scan_error.connect(signal_receiver.on_error)
...
def test_search_worker_rust_mode(qtbot, signal_receiver):
    ...
    with patch(f"{ENGINE_MODULE}.HAS_RUST_ENGINE", True):
        with patch(f"{ENGINE_MODULE}.search_directory_fast", return_value=mock_results) as mock_search:
            ...
            worker.results_found.connect(signal_receiver.on_results_found)
            worker.search_finished.connect(signal_receiver.on_search_finished)
...
def test_search_worker_rust_with_excel(qtbot, signal_receiver):
    ...
    # 2. Excel results
    mock_excel_res = {
        "results": [("C:/test/data.xlsx", 1, [("Sheet1!A1", "keyword")])],
        "skipped": [],
    }
    ...
    with patch(f"{ENGINE_MODULE}.HAS_RUST_ENGINE", True):
        with (
            ...
            patch("core.worker.search_in_files_batch", return_value=mock_excel_res),
        ):
