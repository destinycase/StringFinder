...
def test_highlight_xml_exact_match():
    """XML 전체 일치 강조 테스트: 속성 값은 강조되고 일반 문장은 제외되어야 함"""
...
def test_highlight_xml_documentation_guard():
    """XML 전체 일치 강조 테스트: documentation 내 일반 문장은 강조되지 않아야 함"""
...
def test_highlight_normal_exact_match():
    """일반 모드 전체 일치 강조 테스트: 식별자 구분자 가드 확인"""
    ...
    # 독립된 id만 강조됨 (identifier나 xs:id의 id는 식별자 문자가 붙어있어 제외)
