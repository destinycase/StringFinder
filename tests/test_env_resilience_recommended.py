...
def test_fallback_exact_match_semantics(tmp_path):
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        result = search_in_file(str(p), "id", special_mode=Constants.MODE_EXACT)
...
def test_rust_search_files_list_excel_panic_marker_maps_to_skipped():
    ...
    fake_engine = types.SimpleNamespace(
        search_files_list=lambda *_args, **_kwargs: ([("panic.xls", [fake_match])], [])
    )
...
def test_mainwindow_qtimer_after_destroy(qtbot):
...
def test_mixed_encodings_and_corrupt_bytes(tmp_path):
    ...
    r1 = search_in_file(str(utf16be_file), "TARGET")
    r2 = search_in_file(str(cp949_file), "TARGET")
    r3 = search_in_file(str(corrupt_file), "TARGET")
