...
- 주요 검증 사항:
  ...
  2. 폴백 모드에서의 이진 파일 placeholder 및 누락 방지 검색(Exact Match) 의미론 일치 여부.
...
def test_parallel_runs_no_state_leak_between_workers(qtbot):
    ...
    def fake_search_directory(paths, *args, **kwargs):
        # [Fix] Worker의 results_callback을 호출하여 결과를 전달해야 함
        results_callback = kwargs.get("results_callback")
        if paths == ["A"]:
            results = [("A/a.txt", [(1, "alpha", None, None)])]
            if results_callback:
                results_callback(results)
            return {"results": results, "skipped": []}
        if paths == ["B"]:
            results = [("B/b.txt", [(1, "beta", None, None)])]
            if results_callback:
                results_callback(results)
            return {"results": results, "skipped": []}
    ...
    with (
        ...
        # [Fix] qtbot을 사용하여 스레드가 완료되고 시그널이 처리될 때까지 대기
        qtbot.waitUntil(lambda: not t1.is_alive() and not t2.is_alive(), timeout=5000)
