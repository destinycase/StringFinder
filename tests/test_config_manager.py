...
def test_new_settings(temp_dir):
    ...
    with patch("os.getenv", return_value=temp_dir):
        ...
        assert cm.config.get(Constants.CONFIG_ENABLE_PROFILER) is False
        assert cm.config.get("lock_dock_layout") is False

        # 수정 및 저장
        cm.config[Constants.CONFIG_ENABLE_PROFILER] = True
        cm.config["lock_dock_layout"] = True
        cm.save_immediately()

        ConfigManager._instance = None
        cm2 = ConfigManager()
        assert cm2.config.get(Constants.CONFIG_ENABLE_PROFILER) is True
...
def test_log_retention_settings(temp_dir):
    ...
    with patch("os.getenv", return_value=temp_dir):
        ...
        assert retention.get("max_files") == 5
...
def test_sanitize_session_name_forbidden_chars(temp_dir):
    ...
    with patch("os.getenv", return_value=temp_dir):
        ...
        for input_name, forbidden_char in test_cases:
            ...
            assert forbidden_char not in sanitized, f"Forbidden char '{forbidden_char}' should be removed from '{input_name}'"
