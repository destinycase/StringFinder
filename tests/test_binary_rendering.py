...
class TestBinaryAndHighlightingFix(unittest.TestCase):
    ...
    def test_highlight_with_special_chars(self):
        ...
        html_result = self.model.data(index, Qt.DisplayRole)
    ...
    def test_binary_file_placeholder(self):
        ...
        try:
            ...
            file_path, count, matches = result
            self.assertEqual(count, 1)
            self.assertEqual(matches[0][1], "[바이너리 파일 내 1개 일치 항목 존재]")
            ...
            html_result = self.model.data(index, Qt.DisplayRole)
            self.assertIn("[바이너리 파일", html_result)
