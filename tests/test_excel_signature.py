from core.search_engine import is_valid_excel_signature, search_in_excel
...
def test_excel_signature_check(tmp_path):
    ...
    res = search_in_excel(str(invalid_file), "test")
    assert res[0] == "SKIPPED"
    assert "시그니처" in res[1]  # Check for error message content
