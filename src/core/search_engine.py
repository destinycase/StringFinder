...
def get_rust_mode_bits(special_mode: Optional[str], exclude_binary: bool = False) -> int:
        ...
        try:
            ...
            if results:
                ...
                for m in results:
                    ...
                    parts = content.split(" | ", 2)
                    if len(parts) >= 3:
                        # 줄번호를 0 하드코딩 -> Rust SearchMatch.line 값 사용
                        # 이전: (0, parts[0], ...) -> UI에서 줄번호가 항상 0으로 표시
                        # 수정: (m.line, parts[0], ...) -> Rust가 보고하는 실제 줄번호 반영
...
def search_directory_fast(
    ...
    **kwargs,
    ...
    try:
        ...
        exclude_binary = bool(kwargs.get("exclude_binary", False))
        mode_bits = get_rust_mode_bits(kwargs.get("special_mode"), exclude_binary=exclude_binary)
...
def find_files_with_keyword_fast(
    ...
    **kwargs,
    ...
    try:
        ...
        exclude_binary = bool(kwargs.get("exclude_binary", False))
        rust_mode_bits = get_rust_mode_bits(special_mode, exclude_binary=exclude_binary)
        try:
            found_ret = find_func(
                search_paths,
                rust_pattern,
