        ...
        try:
            ...
            with os.scandir(folder) as entries:
                for entry in entries:
                    ...
                    # [GIL] 대규모 스캔 중 UI 스레드 반응성을 위해 아주 짧게 양보
                    # 약 1000개 파일마다 한 번씩만 양보하여 성능 저하를 최소화합니다.
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            if header.startswith(b"\xd0\xcf\x11\xe0"):
                return True
            # ZIP 기반 (XLSX, XLSM, XLSB, ODS) - PK\x03\x04로 시작
    ...
    try:
        ...
        for sheet_name in workbook.sheet_names:
            try:
                ...
                # sheet.to_python()은 모든 데이터를 리스트의 리스트로 반환
                for row_idx, row in enumerate(sheet.to_python()):
                    for col_idx, cell_value in enumerate(row):
                        if cell_value is not None:
                            ...
                            is_match = False
                            if exact_match:
                                # 전체 일치: 공백 정규화된 값끼리 비교
                                # [Fix] 띄어쓰기 차이로 인한 불일치 방지 (모든 공백 제거 후 비교 추가)
                                is_match = (val_norm == search_string_norm) or (
                                    val_norm.replace(" ", "") == search_string_norm.replace(" ", "")
                                )
                            else:
                                # 부분 일치: 공백 정규화 비교 OR 단순 소문자 포함 비교 (기존 방식 호환)
                            ...
                            if is_match:
                                ...
                                # UI 포맷에 맞게 (dummy_line, location, value) 튜플 저장
                                matches.append((0, pos, val_str))
            except Exception:
                # 특정 시트 처리 중 에러 발생 시 해당 시트 건너뜀 (전체 실패 방지)
                continue
        ...
        # Rust Panic 등 예외를 안전하게 문자열로 변환하여 리턴
        return (Constants.STATUS_SKIPPED, f"Excel Process Error: {e}")
    except BaseException as e:
        # [Critical] Rust Panic 등 일반 Exception으로 잡히지 않는 예외 처리
        logger.critical(f"Critical Excel Special Error (Panic): {e}", exc_info=True)
        return (Constants.STATUS_SKIPPED, f"Critical Process Error: {e}")
...
def search_in_file(
    ...
    if ext in [".xlsx", ".xlsm", ".xls", ".xlsb"]:
        # [Fix] 엑셀은 모드 상관없이 항상 Special Search 로직(공백 정규화 등)을 사용하여 검색 품질 향상
        # 일반 모드일 경우 부분 일치(is_exact=False)로 동작
        is_exact = False
