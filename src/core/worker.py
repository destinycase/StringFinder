...
class GlobalExecutor:
    ...
    def shutdown(cls, wait=False, cancel_futures=True):
        """executor를 강제 종료하고 하위 프로세스를 정리합니다."""
        if cls._executor:
            try:
        ...
        try:
            ...
            # 현재 프로세스의 자식 프로세스만 조회
            active_children = multiprocessing.active_children()

            if active_children:
                logger.debug(f"Cleaning up {len(active_children)} child processes...")

                # 1. Terminate 시도 (SIGTERM)
                ...
                while time.time() - start < 0.5:
                    if not any(p.is_alive() for p in active_children):
                        break
                    time.sleep(0.05)

                # 3. Kill 시도 (SIGKILL / Terminate)
                for p in active_children:
                    if p.is_alive():
                        try:
                            p.kill()  # Python 3.7+
