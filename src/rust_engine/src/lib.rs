// #![allow(dead_code)] 제거: clippy -D warnings 해소를 위해 미사용 실제 실체에만 개별 적용
// [Fix 2-H] 전역 allow 구제: 유지보수 실수로 새 미사용 코드가 CI를 통과하는 것을 방지
mod archive_search;
mod excel_search;
mod json_search;
mod types;
...
const MAX_DECODE_BUFFER_SIZE: usize = 200 * 1024 * 1024; // 200MB Buffer Limit
...
// [Fix 2-H] Python 쪽에서 문자열 비교에 사용되므로 보관 (Rust 코드 내 직접 호출 없어 경고 억제)
#[allow(dead_code)]
...
fn search_file(
    ...
    stop_event: Option<pyo3::PyObject>,  // [Fix 2-B] stop_event 파라미터 추가 (Python 측에서 중단 가능)
) -> PyResult<Vec<SearchMatch>> {
    let (is_json, is_xml, is_archive, is_exact, is_excel) = parse_search_mode(mode_bits);
    ...
    // [Fix 2-B] stop_event를 파이썬에서 수신하여 실제 AtomicBool 플래그로 변환
    // 이전: dummy_stop(항상 false)만 사용 → 단일 파일 검색은 중단 불가
...
fn do_search_with_mmap(
    ...
    if encoding == UTF_8 {
        // [중] 거대 UTF-8 파일 지원: 100MB 초과 시 라인 정렬 청크로 처리하여 메모리 스파이크 방지
            ...
            while current_pos < mmap.len() {
                ...
                if is_exact {
                    ...
                    for mat in ac.find_iter(chunk_data) {
                        ...
                        // [Fix 1-B] last_line_start를 슬라이스 절대 offset으로 정확히 추적
                        // 이전 버그: last_line_start += i + 1 (i는 슬라이스 상대 인덱스 -> 누적 오차)
                        // 수정: 슬라이스 탐색 후 실제 절대 위치 = last_line_start + i + 1 로 갱신
                        for i in 0..m_start.saturating_sub(last_line_start) {
                            if chunk_data[last_line_start + i] == b'\n' {
                                line_number += 1;
                                last_line_start = last_line_start + i + 1;
                            }
                        }
                        // [Fix 외부-중-재스캔] rposition/position 재스캔 제거:
        ...
        if is_exact {
            ...
            for (mat_count, mat) in ac.find_iter(data).enumerate() {
                ...
                for &b in &data[last_line_start..match_start] {
                    if b == b'\n' { line_number += 1; }
                }
                ...
                if results.last().is_none_or(|m: &SearchMatch| m.line != line_number) {
                    results.push(SearchMatch::new(line_number, String::from_utf8_lossy(&data[line_start..line_end]).trim().to_string(), Some(match_start), Some(mat.len())));
                }
                last_line_start = match_start;
            }
        }
    } else {
                ...
                if let Some(last_nl) = decoded_buf.rfind('\n') {
                    ...
                    if is_exact {
                        ...
                        for mat in ac.find_iter(search_range) {
                            let m = mat.start();
                            for c in search_range[last_line_idx..m].chars() {
                                if c == '\n' { line_number += 1; last_line_idx = m; }
                            }
        ...
        if is_exact {
            ...
            for mat in ac.find_iter(content.as_str()) {
                let match_start = mat.start();
                for (i, c) in content[last_line_idx..match_start].char_indices() {
                    if c == '\n' { line_number += 1; last_line_idx += i + 1; }
                }
                let le = content[match_start..].find('\n').map(|i| match_start + i).unwrap_or(content.len());
                if results.last().is_none_or(|r: &SearchMatch| r.line != line_number) {
                    results.push(SearchMatch::new(line_number, content[last_line_idx..le].trim().to_string(), None, None));
                }
...
fn search_file_internal(params: InternalSearchParams) -> Option<Result<Vec<SearchMatch>, String>> {
    ...
    // [중] 파일 타입별 전용 검색기 라우팅
    // [Fix 1-A] archive 전용 분기 추가 (기존 코드에서 누락되어 JSON 라인 전체가 컬럼 1개로 들어오던 버그 수정)
    let res = if is_archive && (ext_lower == ".archive" || ext_lower == ".sf_archive") {
        search_archive_file(&mmap, pattern, ac, is_exact, stop_flag.clone())
    } else if is_json && ext_lower == ".json" {
        if mmap.len() as u64 > MAX_JSON_SIZE {
            vec![SearchMatch::new(
                1,
                encode_skip_reason(REASON_ERR_MEMORY_GUARD, format!("Size {} bytes exceeds limit", mmap.len())),
                None, None,
            )]
        } else {
            search_json_file(&mmap, pattern, ac, is_exact, stop_flag.clone())
        }
    } else if is_xml && ext_lower == ".xml" {
        search_xml_file(&mmap, pattern, ac, is_exact, stop_flag.clone())
    } else if is_binary(&mmap) {
        let count = ac.find_iter(&*mmap).count();
        if count > 0 {
        ...
        do_search_with_mmap(&mmap, pattern, ac, is_exact, &stop_flag)
    };
            ...
            while !stop_flag_mon.load(Ordering::Relaxed) {
                let is_stopped = Python::with_gil(|py| {
                    ...
                    if let Some(cb) = &progress_callback_mon {
                        ...
                        // [중] 성능 최적화: 100건 단위로 배치하여 Python/GIL 부하 감소
                        if count.is_multiple_of(100) || stop_flag_mon.load(Ordering::Relaxed) {
                        ...
                        if entry.file_type().is_some_and(|ft| ft.is_file()) {
                            ...
                            if let Some(ref valid_exts) = exts_ptr {
                                if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
                                    // [중] 성능 최적화: valid_exts는 이미 set_dir 루프 외부에서 to_lowercase() 처리됨
...
fn search_files_list(
    ...
    py.allow_threads(|| {
        ...
        // [Fix 2-D] 매 호출마다 새 rayon 풀 생성 대신 rayon 글로벌 기본 풀 직접 사용
        // 이전: ThreadPoolBuilder::new().build() -> 관리 스레드 + 실제 작업 스레드 이중 생성 오버헤드
        // 수정: rayon 글로벌 풀은 자동으로 thread::available_parallelism()으로 관리됨
        file_list.into_par_iter().for_each(|path_str| {
                ...
                let mut collector = ThreadCollector {
                    local_matches: Vec::new(),
                    local_skipped: Vec::new(),
                    global_matches: matches.clone(),
                    global_skipped: skipped.clone(),
                };
                    ...
                    Ok(file) => {
                        ...
                        match unsafe { Mmap::map(&file) } {
                            Ok(mmap) => {
                                ...
                                // [중] 파일 타입별 전용 검색기 라우팅
                                // [Fix 1-A] archive 전용 분기 추가 (search_file_internal과 동일하게 동기화)
                                let res = if is_excel || ["xlsx", "xlsb", "xls", "xlsm"].contains(&ext.as_str()) {
                                    search_excel_file(path, &search_string, &ac, is_exact, stop_flag.clone())
                                } else if is_archive && (ext_lower == ".archive" || ext_lower == ".sf_archive") {
                                    search_archive_file(&mmap, &search_string, &ac, is_exact, stop_flag.clone())
                                } else if is_json && ext_lower == ".json" {
                                    if mmap.len() as u64 > MAX_JSON_SIZE {
                                        vec![SearchMatch::new(1, encode_skip_reason(REASON_ERR_MEMORY_GUARD, format!("Size {} bytes exceeds limit", mmap.len())), None, None)]
                                    } else {
                                        search_json_file(&mmap, &search_string, &ac, is_exact, stop_flag.clone())
                                    }
                                } else if is_xml && ext_lower == ".xml" {
                                    search_xml_file(&mmap, &search_string, &ac, is_exact, stop_flag.clone())
                                } else if is_binary(&mmap) {
                                    let count = ac.find_iter(&*mmap).count();
                                    if count > 0 {
                                        vec![SearchMatch::new(1, format!("{}{}", MATCH_META_BINARY_PREFIX, count), None, Some(count))]
                                    ...
                                    do_search_with_mmap(&mmap, &search_string, &ac, is_exact, &stop_flag)
                                };

                                if !res.is_empty() {
                                    collector.local_matches.push((path_str.clone(), res));
                                }
                            }
                            Err(e) => {
                                collector.local_skipped.push((
                                    path_str.clone(),
                                    encode_skip_reason(REASON_ERR_MMAP, e),
                                ));
                            }
                        }
                    }
                    Err(e) => {
                        collector.local_skipped.push((path_str.clone(), encode_skip_reason(REASON_ERR_OPEN, e)));
                    }
                }
...
// [Fix 2-H] 미래 fast-path 검색 최적화를 위해 보관 중 (clippy 경고 개별 억제)
#[allow(dead_code)]
...
fn find_files_with_keyword(
    ...
    py.allow_threads(|| {
        let (is_json, is_xml, is_archive, is_exact, _is_excel) = parse_search_mode(mode_bits);
        ...
        let ac = AhoCorasickBuilder::new()
            .ascii_case_insensitive(true)
            .match_kind(MatchKind::Standard) // Fast boolean check
            .build(&patterns)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("{}", e)))?;
                        ...
                        Ok(file) => {
                            ...
                            match unsafe { Mmap::map(&file) } {
                                Ok(mmap) => {
                                    ...
                                    let is_match = match std::panic::catch_unwind(move || {
                                        if let Some(ext) =
...
fn sf_engine(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // [Fix] Rust 엔진 파닉 발생 시 Python 인터프리터 보호를 위한 커스텀 훅 등록
