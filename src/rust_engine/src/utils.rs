...
pub fn detect_encoding(data: &[u8]) -> &'static Encoding {
    ...
    // 좀 더 명확한 구분을 위해 시도
    let (_res, _, has_error) = EUC_KR.decode(data);
    if !has_error {
        return EUC_KR;
    }
    
    // 기본값으로 UTF-8 반환 (혹은 가장 확률 높은 인코딩)
    UTF_8
}
...
// [Fix 2-H] get_line_number: 코드에서 미사용 -> clippy 정리를 위해 제거
// 필요 시 lib.rs의 do_search_with_mmap나 search_file_internal에서 직접 코드 삽입

pub fn parse_search_mode(mode_bits: Option<u32>) -> (bool, bool, bool, bool, bool) {
...
// [Fix 2-H] regex::Regex, std::sync::OnceLock 미사용 import 제거 (clippy)
