...
pub fn search_json_file(
    ...
    if !ctx.is_exact && ctx.ac.find(mmap).is_none() {
        return Vec::new();
    }

    // [개선] 스트리밍 데시리얼라이저를 사용하여 이터레이터 방식으로 접근 시도
    // 단일 거대 객체(100MB급) 처리 시에도 DOM 구축 오버헤드를 분산할 수 있는 여지를 확보합니다. (외부 리뷰 제언 반영)
                ...
                while let Some((val, path)) = stack.pop() {
                    match val {
                        ...
                        JsonValue::String(s) => {
                            ...
                            if is_match {
                                // [최적화] windows().position() 방식의 O(N*M) 검색은 대용량 파일에서 성능 트랩이 됨
...
pub fn check_json_file(
    ...
    // [최적화] 전체 바이트 스캔을 통한 선행 필터링 (파싱 오버헤드 방지)
    if !is_exact && ac.find(mmap).is_none() {
        return false;
    }

    // [개선] 스트리밍 방식으로 구조 탐색하여 메모리 스파이크 억제
    let deserializer = serde_json::Deserializer::from_slice(mmap);
    let iter = deserializer.into_iter::<JsonValue>();
