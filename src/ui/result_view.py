...
class ResultView(QWidget):
    ...
    def _init_ui(self):
        ...
        result_list_layout.setContentsMargins(5, 5, 5, 5)  # Changed from (0, 0, 0, 0) to (5, 5, 5, 5)
        ...
        # [Fix] 검색 모드에 따라 필터링 대상 컬럼이 달라지므로 전용 핸들러로 연결
    ...
    def _on_match_column_resized(self, index, old_width, new_width):
        ...
        # [Fix] Stretch 모드는 레이아웃 엔진에 의해 폭이 자동 결정되므로 저장 대상에서 제외 (진동 루프 방지)
    ...
    def set_search_context(self, search_text, search_mode):
        ...
        # [Fix] UI 콤보박스 값("끄기")을 내부 상수(Constants.MODE_NORMAL)로 정규화
    ...
    def _adjust_match_column_widths(self):
        ...
        # [Fix] 뷰가 가시 상태가 아니더라도 폭 계산이 필요한 경우가 있으므로 체크 완화
        ...
        try:
            ...
            if self.search_mode == Constants.MODE_NORMAL:
                ...
                # [수정] 파일 변경 시 자동 조절이 우선되도록 기존 고정 폭 복원 로직 제거
