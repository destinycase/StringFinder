...
class SearchTab(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # 3단 구성을 위해 result_view, match_area_widget, preview_group을 직접 splitter에 추가
        ...
        self.result_splitter.setStretchFactor(2, 1)
    ...
    def start_search(self):
        ...
        # 모든 필터 조건이 수집된 후 통합 로그 출력
        filename_log_text = ", ".join(selected_filenames) if selected_filenames else ""
        logger.info(AppStrings.LOG_SCH_STARTED.format(search_text, filename_log_text))
        ...
        if hasattr(main_window, "profiling_widget"):
            main_window.profiling_widget.start_profiling()
        else:
            # window()로 안 잡힐 경우를 대비해 부모를 탐색합니다.
            curr = self.parent()
            while curr:
                if hasattr(curr, "profiling_widget"):
                    curr.profiling_widget.start_profiling()
                    break
                curr = curr.parent()
        ...
        if use_rust_fast_path:
            logger.info(AppStrings.LOG_SCH_FAST_PATH.format(selected_folders, selected_exts))
            # [Step 1] 고성능 엔진에서는 스캔과 검색이 통합되어 진행됨을 로그로 명시
            # 스캔 자체는 매우 순식간에 일어나므로 측정된 시간을 로그에 반영
            scan_duration = time.time() - self.start_timer
            logger.info(AppStrings.LOG_SCH_SCAN_DONE_SIMPLE.format(scan_duration))
        ...
        self.scan_worker = ScanWorker(selected_folders, selected_exts, selected_filenames, search_text)
        self.scan_worker.moveToThread(self.scan_thread)

        self.scan_thread.started.connect(self.scan_worker.run)
        self.scan_worker.scan_finished.connect(
            lambda files: self._on_scan_finished(files, search_text, selected_folders, selected_filenames)
        )
    ...
    def _on_scan_thread_finished(self):
        ...
        # [3-State Button] 중단된 상태에서 스캔이 끝났거나, 검색으로 이어지지 않는 경우 버튼 복원
        if not self.worker:
            self._restore_search_button()
            # 검색 워커로 넘어가지 못하고 스캔에서 중단된 경우, 상태를 초기화합니다.
            self.status_message_requested.emit(AppStrings.STATUS_READY, 0)

            # [State Machine] 스캔 중단 -> IDLE
            self.search_state = Constants.SearchState.IDLE
            self._check_pending_restart()

        # 참조는 worker_finished 가 아닌 전용 finished 핸들러가 없으므로 여기서 정리하되,
        # worker_finished에서도 정리하므로 중복 방지를 위해 안전하게 처리
        # 참조 정리는 스레드가 완전히 종료된 시점에 수행합니다.
        self.scan_worker = None
        self.scan_thread = None

        # [State Machine] 스캔 스레드 종료 -> IDLE
        # 단, 스캔이 끝나고 바로 검색 워커로 넘어가는 성공 케이스(_on_scan_finished 호출됨)에서는
        # 상태가 SEARCHING으로 바뀌므로, 여기서 무조건 IDLE로 바꾸면 안됨.
        # 따라서 "검색 워커로 넘어가지 않은 경우"에만 IDLE로 천이해야 함.
    ...
    def _setup_search_worker(self, params):
        ...
        self.worker.finished.connect(self.thread.quit)  # [Fix] 스레드 종료 연결 추가
    ...
    def _on_worker_finished(self):
        ...
        # [3-State Button] 모든 작업이 끝났으므로 버튼 상태 복구
        self._restore_search_button()
    ...
    def _on_progress(self, current, total):
        ...
        target_widget = None
        if hasattr(main_window, "profiling_widget"):
            target_widget = main_window.profiling_widget
        else:
            curr = self.parent()
            while curr:
                if hasattr(curr, "profiling_widget"):
                    target_widget = curr.profiling_widget
                    break
                curr = curr.parent()

        if target_widget:
            target_widget.update_progress(current, self.total_matches)
    ...
    def _on_search_finished(self, found_count, skipped_count):
        ...
        main_window = self.window()
        if hasattr(main_window, "profiling_widget"):
            main_window.profiling_widget.stop_profiling()
        else:
            curr = self.parent()
            while curr:
                if hasattr(curr, "profiling_widget"):
                    curr.profiling_widget.stop_profiling()
                    break
                curr = curr.parent()
