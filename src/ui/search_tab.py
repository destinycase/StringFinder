...
class SearchTab(QMainWindow):
    ...
    def _refresh_logs_output(self, _state: Optional[int] = None):
        # [최적화] 대량 로그 필터링 시 제너레이터를 사용하여 메모리 할당 및 중간 객체 생성 최소화
    ...
    def _on_log_message(self, level: str, message: str):
        # [개선] 테스트 환경에서는 즉시 플러시하여 UI 검증 로직이 깨지지 않도록 함
    ...
    def _flush_pending_logs(self):
        ...
        for level, message in current_batch:
            ...
            if normalized_level in visible_levels:
                logs_to_show.append(str(message))
        ...
        if logs_to_show:
            ...
            if len(logs_to_show) > MAX_BATCH_LINES:
                truncated_logs = logs_to_show[:MAX_BATCH_LINES]
                truncated_logs.append(AppStrings.MSG_MATCH_TRUNCATION_NOTICE)
    ...
    def get_state(self):
        ...
        # [수정] 상태 저장 전 보류 중인 로그가 있다면 강제로 UI에 플러시하여 직렬화 누락 방지
        self._flush_pending_logs()
    ...
    def _on_scan_finished(self, file_list):
        ...
        selected_filenames = getattr(self.scan_worker, Constants.PAYLOAD_FILENAME_FILTER, None)
        ...
        search_params = {
            ...
            Constants.PAYLOAD_EXTENSIONS: self.scan_worker.selected_exts if self.scan_worker else [],
    ...
    def _on_worker_finished(self):
        ...
        try:
            ...
            self._restore_search_button()
            self.status_message_requested.emit(AppStrings.STATUS_READY, 0)
            # [중] 중지 중이거나 검색 중일 때 모두 종료 시 IDLE 복구 보장
            if self.search_state in (Constants.SearchState.SEARCHING, Constants.SearchState.STOPPING):
                self.search_state = Constants.SearchState.IDLE
                self._check_pending_restart()
        except Exception as e:
    ...
    def _on_search_finished(self, found_count, skipped_count):
        ...
        # [Fix] Phase 1(스캔)과 Phase 2(검색) 단계의 모든 스킵 파일을 합산하여 요약에 표시
