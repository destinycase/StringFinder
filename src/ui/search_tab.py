...
from PySide6.QtCore import Qt, QThread, QByteArray, Signal, QTimer
...
class SearchTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.search_combo.history_item_deleted.connect(lambda t: self._remove_history_item(t, AppStrings.TYPE_SEARCH))
        self.search_combo.history_cleared.connect(lambda: self._clear_history(AppStrings.TYPE_SEARCH))
        ...
        self.filename_combo.history_item_deleted.connect(
            lambda t: self._remove_history_item(t, AppStrings.TYPE_FILENAME)
        )
        self.filename_combo.history_cleared.connect(lambda: self._clear_history(AppStrings.TYPE_FILENAME))
        ...
        self.folder_select_all_btn.clicked.connect(lambda: self._toggle_all_filters(AppStrings.TYPE_FOLDER, True))
        self.folder_deselect_all_btn = QPushButton(AppStrings.DESELECT_ALL_BTN)
        self.folder_deselect_all_btn.setFixedWidth(80)
        self.folder_deselect_all_btn.clicked.connect(lambda: self._toggle_all_filters(AppStrings.TYPE_FOLDER, False))
        ...
        self.ext_select_all_btn.clicked.connect(lambda: self._toggle_all_filters(AppStrings.TYPE_EXT, True))
        self.ext_deselect_all_btn = QPushButton(AppStrings.DESELECT_ALL_BTN)
        self.ext_deselect_all_btn.clicked.connect(lambda: self._toggle_all_filters(AppStrings.TYPE_EXT, False))
        ...
        self._restore_column_widths(AppStrings.VIEW_RESULT)
        self.result_view.horizontalHeader().setStretchLastSection(False)  # Stretch 모드 직접 제어를 위해 False
        self.result_view.horizontalHeader().sectionResized.connect(
            lambda i, o, n: self._save_column_widths(AppStrings.VIEW_RESULT)
        )
        ...
        # 파일 내 구체적인 매칭 지점들을 보여주는 테이블 뷰 및 필터
        ...
        self.match_filter_1_edit = QLineEdit()  # 첫 번째 필터 (이름/키 등)
        self.match_filter_2_edit = QLineEdit()  # 두 번째 필터 (내용/수치 등)
        ...
        # 초기 컬럼 너비 복원 및 리사이즈 모드 설정
        self._restore_column_widths(AppStrings.VIEW_MATCH)
        self.match_view.horizontalHeader().setStretchLastSection(False)  # Stretch 모드 직접 제어를 위해 False
        self.match_view.horizontalHeader().sectionResized.connect(
            lambda i, o, n: self._save_column_widths(AppStrings.VIEW_MATCH)
        )
        ...
        self.preview_text.setStyleSheet(AppStrings.STYLE_PREVIEW_EMPTY)

        preview_layout.addWidget(self.preview_text)

        # 스플리터 계층 조립
        self.bottom_splitter.addWidget(self.match_area_widget)
        ...
        # 결과 탭 메인 레이아웃 구성 (필터 입력 레이아웃 + 스플리터)
        ...
        try:
            ...
            if hasattr(self, "scan_thread") and self.scan_thread:
                if self.scan_thread.isRunning():
                    ...
                    if not self.scan_thread.wait(5000):  # 5초 타임아웃
                        logger.warning(AppStrings.LOG_THREAD_TIMEOUT_SCAN)
                        self.scan_thread.terminate()
                        self.scan_thread.wait(1000)
        except RuntimeError:
            # C++ 객체가 이전에 삭제된 경우 예외 무시
            pass
        ...
        try:
            ...
            if hasattr(self, "thread") and self.thread:
                if self.thread.isRunning():
                    self.thread.quit()
                    if not self.thread.wait(5000):  # 5초 타임아웃
                        logger.warning(AppStrings.LOG_THREAD_TIMEOUT_SEARCH)
    ...
    def start_search(self):
        ...
        if special_mode == AppStrings.SPECIAL_SEARCH_OFF:
            ...
            if AppStrings.MODE_XML in special_mode:
                selected_exts = [".xml"]
            elif AppStrings.MODE_JSON in special_mode:
                selected_exts = [".json"]
            else:
        ...
        # 조건: Rust 모듈 로드됨 + 특수모드 꺼짐 + 파일명 필터 없음 (현재 Rust 구현은 확장자 필터만 지원)
        from core.search_engine import HAS_RUST_ENGINE

        use_rust_fast_path = HAS_RUST_ENGINE and special_mode == AppStrings.SPECIAL_SEARCH_OFF and not filename_filter

        if use_rust_fast_path:
            logger.info(AppStrings.LOG_RUST_FAST_PATH.format(selected_folders, selected_exts))
            # [Step 1] 고성능 엔진에서는 스캔과 검색이 통합되어 진행됨을 로그로 명시
            logger.info(AppStrings.LOG_SCAN_COMPLETED.format("고속 통합 스캔", 0.000))
            self.scan_start_time = time.time()  # 시간 측정 호환성
            ...
            # Rust 모드에서는 search_finished만 호출됨 (scan_finished 없음)
            ...
            self.thread.finished.connect(self._on_search_thread_finished)

            self.skipped_files_list = []
            self.thread.start()
            return

        # 3. [Legacy] 파일 목록 스캔 단계 (Python)
        self.scan_thread = QThread()
        self.scan_worker = ScanWorker(selected_folders, selected_exts, filename_filter, search_text)
    ...
    def _on_scan_finished(self, file_list, search_text, selected_folders):
        ...
        if not file_list:
            ...
            # 에러/상태 안내용 스타일 적용
            self.empty_label.setStyleSheet(AppStrings.STYLE_EMPTY_LABEL.format(AppStrings.COLOR_RED))
            self.empty_label.setVisible(True)
    ...
    def _on_search_finished(self, found_count, skipped_count):
        ...
        if hasattr(self, "skipped_files_list") and self.skipped_files_list:
            ...
            for item in self.skipped_files_list:
                if isinstance(item, tuple) and len(item) == 2:
                    f_path, reason = item
                    logger.warning(AppStrings.LOG_SKIP_REASON.format(f_path, reason))
                else:
                    logger.warning(AppStrings.LOG_SKIP_SIMPLE.format(item))

        # logger.info(AppStrings.LOG_WORKER_FINISHED... ) -> Worker에서 이미 출력하므로 중복 제거
        ...
        status_msg = AppStrings.STATUS_SEARCH_COMPLETED.format(
            ...
            self.total_files,
            self.total_matches,
            len(self.skipped_files_list) if hasattr(self, "skipped_files_list") else 0,
            elapsed,
        )
    ...
    def _update_match_filter_ui(self, mode):
        ...
        if "XML" in mode:
            ...
            self.match_filter_1_edit.setVisible(False)
            self.match_filter_2_edit.setPlaceholderText(AppStrings.MATCH_FILTER_CONTENT_PLACEHOLDER)
            self.match_filter_2_edit.textChanged.disconnect()
