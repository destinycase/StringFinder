...
class SearchTab(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # 3단 구성을 위해 result_view와 pagination을 하나의 위젯으로 묶음 (스플리터 제거)
        ...
        # 스트레치 비율: result_list_container(1), match_area(1), preview(1)
    ...
    def _on_scan_finished(self, file_list):
        ...
        # [Fix] 워커로부터 검색 당시의 파라미터 복원
        search_text = self.scan_worker.search_string
        selected_folders = self.scan_worker.selected_folders
        # filename_filter는 ScanWorker 내부에서 이미 처리되었으나,
        # 다음 단계(SearchWorker)나 UI 표시를 위해 필요하다면 가져옴
    ...
    def _show_match_context_menu(self, pos):
        ...
        if not index.isValid():
            ...
            # [Fix] DisplayRole(HTML) 대신 EditRole(Raw Text)을 사용하여 태그 없는 순수 텍스트 복사
        content = self.match_view.model().data(index, Qt.EditRole)
    ...
    def _setup_copy_shortcuts(self):
        """Ctrl+C 단축키를 테이블 뷰에 연결하여 사용자 정의 복사 동작을 수행합니다."""
        from PySide6.QtGui import QShortcut, QKeySequence

        # Result View (파일 목록) 복사 단축키
        self.copy_result_shortcut = QShortcut(QKeySequence.Copy, self.result_view, context=Qt.WidgetShortcut)
        self.copy_result_shortcut.activated.connect(self._copy_selected_result_path)

        # Match View (상세 결과) 복사 단축키
        self.copy_match_shortcut = QShortcut(QKeySequence.Copy, self.match_view, context=Qt.WidgetShortcut)
    ...
    def _on_match_filter_3_changed(self, text):
        """세 번째 필터 입력 변경 시 처리"""
        # mode = self.special_search_combo.currentText()  # Unused
        col = 4  # Archive mode translation
        self.match_proxy_model.setColumnFilter(col, text)
