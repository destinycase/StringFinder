...
class SearchTab(QMainWindow):
    ...
    def start_search(self):
        ...
        try:
            ...
            self.scan_worker = ScanWorker(
                ...
                use_complex_search=self.search_panel.is_complex_search(),
            )
            ...
            self.scan_worker.signals.skipped_found.connect(self._on_skipped_found)
            self.scan_worker.signals.error.connect(self._on_scan_error)
    ...
    def _on_scan_thread_finished(self):
        ...
        try:
            ...
            logger.error(AppStrings.LOG_SCH_SCAN_THREAD_ERROR.format(e))
        finally:
    ...
    def _on_scan_finished(self, file_list):
        ...
        if self.scan_worker is None:
            ...
            return
        # Capture self.scan_worker atomically to avoid TOCTOU race.
        # Between the None check above and attribute access below, another thread
        # could set self.scan_worker = None in _on_scan_thread_finished.
        _sw = self.scan_worker
        if _sw is None:
            logger.error("[H-03] scan_worker became None between guard and use.")
            return
        search_text = _sw.search_string
        selected_folders = _sw.selected_folders
        selected_filenames = getattr(_sw, Constants.PAYLOAD_FILENAME_FILTER, None)
        if selected_filenames is None:
            selected_filenames = getattr(self, "current_filename_filters", [])
        self.result_view_panel.result_model.set_filename_filters(selected_filenames)
        # ScanWorker 내에서 상세 로그를 출력하므로 여기서는 생략합니다.
        ...
        if not file_list:
            ...
            if not selected_folders:
                msg = AppStrings.RESULT_EMPTY_NO_FOLDER
            else:
                msg = AppStrings.RESULT_EMPTY_NO_MATCH.format(search_text)
            self.result_view_panel.show_empty_message(msg, is_error=True)
            self._restore_search_button()
            self.status_message_requested.emit(AppStrings.STATUS_READY, 0)
        ...
        special_mode_val = self.ext_panel.get_special_mode()
        ...
        search_params = {
            Constants.PAYLOAD_FILE_LIST: file_list,
            Constants.PAYLOAD_SEARCH_STRING: search_text,
            Constants.PAYLOAD_SPECIAL_MODE: final_special_mode,
            Constants.PAYLOAD_FILENAME_FILTER: selected_filenames,
            Constants.PAYLOAD_SEARCH_PATHS: selected_folders,
            Constants.PAYLOAD_EXTENSIONS: _sw.selected_exts if _sw else [],  # Constants.PAYLOAD_USE_COMPLEX_SEARCH: self.search_panel.is_complex_search(),
            Constants.PAYLOAD_EXCLUDE_BINARY: self.search_panel.is_exclude_binary(),
        }
    ...
    def _on_results_found(self, results):
        ...
        if results:
            self.result_view_panel.add_results(results)
            self.total_files = len(self.result_view_panel.result_model.get_all_results())
