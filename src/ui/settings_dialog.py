...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # 5. 로그 보존 설정
        log_retention_label = QLabel("로그 파일 보존")
        log_retention_label.setStyleSheet("font-weight: bold;")
        layout.addWidget(log_retention_label)

        # 로그 보존 활성화 체크박스
        self.log_retention_checkbox = QCheckBox("로그 파일 보존 활성화")
        retention_config = self.config_manager.get("log_retention", {})
        self.log_retention_checkbox.setChecked(retention_config.get("enabled", False))
        self.log_retention_checkbox.stateChanged.connect(self._on_log_retention_changed)
        layout.addWidget(self.log_retention_checkbox)

        # 최대 파일 수 설정
        max_files_layout = QHBoxLayout()
        max_files_label = QLabel("최대 보관 파일 수:")
        ...
        self.max_files_spinbox.setEnabled(retention_config.get("enabled", False))
        ...
        max_days_label = QLabel("최대 보관 일수:")
        ...
        self.max_days_spinbox.setEnabled(retention_config.get("enabled", False))
    ...
    def _on_log_retention_changed(self, state):
        """로그 보존 설정 변경 시 호출"""
    ...
    def _open_data_dir(self):
        ...
        if os.path.exists(data_dir):
            try:
                ...
                logger.error(AppStrings.ERROR_OPEN_DATA_DIR.format(e))
                QMessageBox.warning(
                    self,
