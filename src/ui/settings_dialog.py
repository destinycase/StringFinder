...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # 캐시 통계
        self.cache_stats_label = QLabel(AppStrings.CACHE_STATS_LABEL)
        performance_layout.addWidget(self.cache_stats_label)

        self.cache_stats_value = QLabel("로딩 중...")
        self.cache_stats_value.setStyleSheet("color: gray; font-size: 10px;")
        performance_layout.addWidget(self.cache_stats_value)
    ...
    def _update_cache_stats(self):
        ...
        try:
            ...
            cache = HybridSearchCache(cache_dir, persist=False)  # 통계만 로드
            cache.load_from_disk()

            stats = cache.get_stats()
            count = stats["result_cache_size"] + stats["file_cache_size"]
            hit_rate = stats["hit_rate"]

            # 메모리 계산 (대략적)
            memory_mb = (count * 0.3) / 1000  # 평균 300KB per item

            stats_text = AppStrings.CACHE_STATS_FORMAT.format(count=count, hit_rate=hit_rate, memory=f"{memory_mb:.1f}")
            self.cache_stats_value.setText(stats_text)
        except Exception as e:
            self.cache_stats_value.setText(f"통계 로드 실패: {e}")
            logger.warning(f"[설정] 캐시 통계 로드 실패: {e}")
    ...
    def _clear_cache(self):
        ...
        reply = QMessageBox.question(
            ...
            "저장된 모든 검색 캐시를 삭제하시겠습니까?\n\n"
            "캐시를 삭제해도 검색 기능에는 영향이 없으며,\n"
            "다음 검색부터 새로운 캐시가 생성됩니다.",
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.information(self, AppStrings.SUCCESS_TITLE, "캐시가 성공적으로 삭제되었습니다.")
