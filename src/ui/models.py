...
class SearchResultModel(QAbstractTableModel):
    ...
    def add_results(self, results):
        ...
        self.beginResetModel()
        for item in results:
            # item이 list 형태([count, name, ...])인 경우와 tuple 형태(file_path, count, matches) 모두 대응
            if len(item) >= 5:
                # 이미 정규화된 데이터 세션 로드 등의 경우
                self._result_buffer.append(list(item))
            elif len(item) >= 3:
                # 워커에서 넘어온 원본 데이터 (file_path, count, matches)
                ...
                self._result_buffer.append([count, file_name_with_ext, folder, file_path, safe_matches])
...
class MatchDetailModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        # [Fix] search_mode가 None일 경우를 대비한 안전 로직
        ...
        if role in [Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.EditRole]:
            if search_mode == Constants.MODE_NORMAL:
                ...
                if is_excel:
                    # 엑셀: [하이라이팅된값]
                    # [Fix] 일반 모드 엑셀의 경우 content 또는 extra_1에 실제 값이 있을 수 있으므로 방어적으로 처리
    ...
    def set_matches(self, file_path, matches, search_text="", search_mode=Constants.MODE_NORMAL):
        ...
        # [중] 버퍼 초기화 보강 (rowCount 불일치 및 메모리 잔존 해결)
        ...
        if matches:
            for m in matches:
                ...
                # Case 1: Excel (Line, Sheet, Cell, Val, [Offset, Length])
                # [수정] 확장자뿐만 아니라 실제 엑셀 모드인 경우에만 엑셀 전용 구조로 파싱
                ...
                # [개선] 일반 모드에서도 엑셀 4-튜플(Line, Sheet, Cell, Value)이 올 경우 위치 정보를 Sheet!Cell 형식으로 조합
                line_no: str = ""
                content: str = ""
                offset: Optional[Union[int, str]] = None
                length: Optional[int] = None

                # [Fix] Rust Excel 결과(6-튜플) 및 Python Excel 결과(4-튜플) 모두 대응
    ...
    def clear(self):
        self.beginResetModel()
        self._data = []
        self._all_data_buffer = []  # [중] 버퍼 누락으로 인한 카운트 불일치 해결
        self.current_file_path = ""
        self.endResetModel()
