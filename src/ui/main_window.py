...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # [v4.33.2 Fix] Disable tray in tests to prevent OS-level hangs
        ...
        self._apply_theme()
        # [v4.33.1 Fix] 시스템 테마 변경 감지 및 자동 적용 (Windows 전용 API 연동)
    ...
    def _quit_application(self):
        ...
        for i in range(self.tab_widget.count()):
            ...
            if isinstance(tab, SearchTab):
                try:
                    # SearchTab has stop_search which calls _stop_existing_search
                    tab.stop_search()
                except Exception as e:
                    logger.warning(AppStrings.LOG_WKR_BATCH_ERROR.format(e))
            elif hasattr(tab, "stop_search"):
                # Fallback for duck typing if needed
                try:
    ...
    def _set_ui_locked(self, locked):
        """
        검색 진행 중 주요 제어(탭 이동/생성/닫기 및 설정)를 잠그거나 해제합니다.

        Args:
            locked (bool): True이면 잠금 (검색 시작 시), False이면 해제 (검색 종료 시)
        """
    ...
    def _apply_theme(self):
        """설정된 테마(Dark/Light)를 바탕으로 애플리케이션 스타일시트를 업데이트합니다."""
        # [v4.33.2 Fix] Disable theme loading in tests to save memory and prevent font/resource leaks
