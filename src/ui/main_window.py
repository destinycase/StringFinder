...
class MainWindow(QMainWindow):
        ...
        if not HAS_RUST_ENGINE:
            ...
            QTimer.singleShot(
                ...
                lambda: self.statusBar().showMessage(
                    "Rust 가속 엔진을 로드할 수 없어 Python 엔진으로 동작합니다. (성능 저하 가능성 있음)",
                    10000,  # 10초간 표시
                ),
            )
    ...
    def _init_ui(self):
        ...
        # 프로파일링 도킹 위젯 추가 (설정에 따라 활성화)
        self.profiling_dock = None
        if self.config_manager.get(Constants.CONFIG_ENABLE_PROFILER, False):
            self._init_profiler_ui()
    ...
    def _init_profiler_ui(self):
        """프로파일러 UI를 초기화하고 도킹합니다."""
        if self.profiling_dock is not None:
            # 이미 생성되어 있으면 창만 표시
            self.profiling_dock.show()
            return

        self.profiling_dock = QDockWidget(AppStrings.DOCK_PROFILER_TITLE, self)
        self.profiling_widget = ProfilingWidget(self.config_manager)
        self.profiling_dock.setWidget(self.profiling_widget)
        self.addDockWidget(Qt.RightDockWidgetArea, self.profiling_dock)
        self.profiling_dock.show()

    def _toggle_profiler_from_settings(self, enabled):
        """설정창에서 프로파일러 상태가 변경되었을 때 호출됩니다."""
        if enabled:
            self._init_profiler_ui()
        else:
            self._close_profiler_ui()

    def _close_profiler_ui(self):
        """프로파일러 UI를 닫고 리소스를 해제합니다."""
        if self.profiling_dock:
            self.profiling_dock.close()
            self.removeDockWidget(self.profiling_dock)
            self.profiling_dock.deleteLater()
            self.profiling_dock = None
            self.profiling_widget = None
