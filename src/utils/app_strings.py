class AppStrings:
    ...
    APP_VERSION = "v4.19.1" # PyInstaller 빌드 시 override됨
    SEARCH_LABEL = "검색 문자열:"
    ADD_TAB_BTN = "+"
    ...
    LOG_PRF_SAVE_FAIL = "[프로파일러][오류] 리포트 저장 실패: {}"
    LOG_PRF_LOAD_FAIL = "[프로파일러][경고] 히스토리 로드 실패: {}"
    ...
    # --- [검색 흐름 로그] ---
    LOG_SCH_STARTED = "[검색] 시작 (검색어: '{}', 파일명 필터: '{}')"
    LOG_SCH_SCAN_DONE = "[검색][Step 1] 파일 스캔 완료: {}개 파일 발견 (소요 시간: {:.3f}초)"
    LOG_SCH_SCAN_DONE_SIMPLE = "[검색][Step 1] 파일 스캔 완료 (소요 시간: {:.3f}초)"

    LOG_SCH_SEARCH_DONE = "[검색][Step 2] 문자열 검색 완료 (소요 시간: {:.3f}초)"
    LOG_SCH_NO_FILES = "[검색] 검색할 파일이 없습니다. UI를 초기화합니다."
    LOG_SCH_EMPTY_QUERY = "[검색][경고] 검색어를 입력해 주세요."

    LOG_SCH_FAST_PATH = "[검색] 검색 경로 진입 (폴더: {}, 확장자: {})"
