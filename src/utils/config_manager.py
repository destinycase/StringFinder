...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self.defaults = {
            ...
            "log_retention": {
                "enabled": True,  # 기본값: 로그 보존 활성화 (사용자 요청)
                "max_files": 10,  # 최대 보관 파일 수
                "max_days": 3,  # 최대 보관 일수 (기본 3일)
            },
            "search_cache": {
                "enabled": False,  # 기본값: 캐시 사용 안 함 (사용자 요청)
                "max_results": 100,  # LRU 캐시 크기
                "persist": True,  # 디스크 저장 여부
            },
    ...
    def clear_all_logs(self):
        ...
        for f in files:
            try:
                ...
                # 현재 사용 중인 로그 파일은 삭제 불가 (무시)
                pass

        logger.info(f"[설정] 로그 파일 {deleted_count}개 삭제 완료")
    ...
    def get_cache_enabled(self) -> bool:
        """검색 캐시 활성화 여부 반환"""
        return self.config.get("search_cache", {}).get("enabled", True)
    ...
    def get_cache_max_results(self) -> int:
        """캐시 최대 크기 반환"""
        return self.config.get("search_cache", {}).get("max_results", 100)

    def get_cache_persist(self) -> bool:
        """캐시 디스크 저장 여부 반환"""
        return self.config.get("search_cache", {}).get("persist", True)
