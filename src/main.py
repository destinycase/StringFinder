...
def main():
    ...
    def global_exception_handler(exctype, value, traceback):
        ...
        logger.critical(AppStrings.LOG_SYS_UNCAUGHT_EXCEPTION, exc_info=(exctype, value, traceback))
    ...
    def cleanup_on_exit():
        ...
        # [Critical] GlobalExecutor 종료 - 모든 워커 프로세스 정리
        # 설정 저장 전에 먼저 실행하여 프로세스가 설정 파일에 접근하지 않도록 함
        try:
            logger.info("[시스템] 워커 프로세스 종료 중...")
            GlobalExecutor.shutdown(wait=True, cancel_futures=True)
            logger.info("[시스템] 워커 프로세스 종료 완료")
        except Exception as e:
            logger.error(f"[시스템] 워커 프로세스 종료 실패: {e}")
...
# [Crash Handler] 전역 예외 처리
def global_exception_handler(exctype, value, tb):
    """
    잡히지 않은 예외를 처리하여 로그 파일과 크래시 덤프에 기록하고,
    사용자에게 알림 메시지를 표시합니다.
    """
    import traceback
    import datetime
    ...
    # 3. 로거에 기록 (로거가 초기화된 경우)
    try:
        import logging

        logging.getLogger("StringFinder").critical("Uncaught exception", exc_info=(exctype, value, tb))
    except Exception:
        pass

    # 4. 크래시 덤프 파일 기록 (실행 파일 위치)
    try:
    ...
    # 5. 사용자 알림 (Native Windows Message Box)
    # Qt가 죽었을 수도 있으므로 ctypes 사용
    try:
        import ctypes
...
if __name__ == "__main__":
    # [Multiprocessing] Windows PyInstaller 필수
    multiprocessing.freeze_support()

    try:
        main()
    except Exception as e:
        # main() 실행 중 밖으로 튀어 나온 예외도 잡기 위함
        # sys.excepthook이 호출되지 않는 경우를 대비
