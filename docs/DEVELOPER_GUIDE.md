...
실무 환경의 성능 병목을 한 번에 수집해야 할 때는 GUI의 **성능 진단** 버튼을 사용합니다. 일반 검색·존재만 확인·누락 방지 검색을 고정 시나리오로 각각 5회 측정하고, 회차별 시간·매치/스킵 집계·오류 판정과 파일 형식·크기 분포·메모리·하드웨어 정보를 JSON과 Markdown으로 생성합니다. 검색어는 리포트에 기록하지 않으며 파일 경로와 파일 내용도 저장하지 않습니다. 결과 파일은 사용자가 선택한 위치에 저장됩니다. CLI에서는 `python tools/diagnostic_benchmark.py <폴더> --repeats 5`로 동일하게 실행할 수 있습니다.
