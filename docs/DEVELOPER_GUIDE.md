...
- **문서 버전:** 1.6 (StringFinder v5.9.19 기준)
- **최종 수정일:** 2026-09-25
- **대상 독자:** 코어 검색 엔진 및 UI/UX 개발자, 기여자(Maintainers & Contributors)

> Documentation baseline: **StringFinder 5.9.19** · Updated: **2026-09-25**
...
- Rust 파일 순회의 `WalkBuilder`는 `.gitignore` 규칙을 검색 제외 조건으로 적용하지 않습니다. `.gitignore` 파일 자체도 일반 파일 후보로 취급하며, 숨김 파일/폴더 제외는 별도 설정으로만 제어합니다. Python 누락 방지 경로와 Rust 일반 검색의 후보 집합이 이 동작에서 일치해야 합니다.
