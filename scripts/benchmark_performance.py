...
def create_dataset():
    ...
    print("[Step 3/3] Creating Set C: Binary Mixed")
...
def run_benchmark(dataset_name, path, keyword):
    """지정된 데이터셋에 대해 벤치마크 수행"""
    app = QApplication.instance()
    if not app:
        app = QApplication(sys.argv)

    print(f"\n[Bench] Running benchmark for {dataset_name}...")
...
def print_result_table(bench_results):
    print("\n" + "="*80)
    print(" PERFORMANCE BENCHMARK REPORT (PERF-06)")
    print("="*80)
    print(f"| {'Dataset':<20} | {'Total Time':<12} | {'Latency':<12} | {'Jitter':<12} | {'Hits':<8} |")
    print(f"| {'-'*20} | {'-'*12} | {'-'*12} | {'-'*12} | {'-'*8} |")
    for res in bench_results:
        print(f"| {res['dataset']:<20} | {res['total_time']:>10.3f}s | {res['latency']:>10.3f}s | {res['jitter']:>10.4f} | {res['results_count']:>8} |")
    print("="*80)

def check_thresholds(results):
    """성능 기준치 (Thresholds) - [REF-PERF-06] CI 자동 게이트용"""
    THRESHOLDS = {
        "Set A: Small/Many": {"total_time": 3.0, "latency": 1.0, "jitter": 0.1},
        "Set B: Mixed/Large": {"total_time": 1.5, "latency": 0.5, "jitter": 0.1},
        "Set C: Binary Mixed": {"total_time": 0.5, "latency": 0.2, "jitter": 0.1},
    }
    ...
    for res in results:
        ...
        if t:
            ...
            if res["jitter"] > t["jitter"]:
                print(f"[FAIL] {res['dataset']}: Jitter {res['jitter']:.3f} > {t['jitter']}")
                failed = True
