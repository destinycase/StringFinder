...
- **고립 정책 (Isolation)**: 일반적인 워크로드에서 불필요한 Python 폴백을 차단하여 검색 무결성과 성능을 동시에 확보했습니다.
- **선별적 폴백**: UTF-16 (BOM) 등 Rust 엔진이 직접 처리하기 어려운 특수 인코딩에 대해서만 지능적으로 Python 폴백을 허용합니다.
...
- **최상위 성능**: Mmap 기반 대용량 파일 고속 매핑 및 하이브리드 디스패처(Spin-drain + Sleep)를 통한 효율적 결과 배출
