...
- 통합 파이프라인: 첫 번째 결과 도달 시간(Zero-TTFR) 최소화를 위해 스캔과 본문 검색을 단일 단계로 오케스트레이션
- 유연한 한계 설정: 사용자 맞춤형 고급 설정(Advanced Settings) UI 제공을 통한 하드웨어 자원 효율적 활용
- 사용자에 따른 특별한 검색 모드 지원
...
- **중앙 설정화**: 사용 빈도와 성능 기여도를 고려하여 해당 옵션을 메인 UI에서 **설정(Settings)** 창으로 이동하고 영속성을 부여했습니다.
- **최상위 성능**: **Mmap 기반 대용량 파일 고속 매핑** 및 `_fast_existence_check`(Negative Filter)를 통한 대용량 구조 파일 초고속 스캔
...
- 문맥 미리보기 전후 표시 줄 수 사용자 설정 및 저장 (각 0~20줄, 기본 5줄)
- 숨김 파일/폴더 제외 옵션
- 결과 내보내기 (`.xlsx`, `.txt`)
- 탭 기반 세션 저장/복원
- 단일 인스턴스 실행 및 윈도우 포커스 제어
...
- **Python 엔진(정밀 검색)**: 유니코드 정규화가 필요한 복합 검색(`use_complex_search=True`) 또는 Rust가 처리 불가능한 특수 인코딩(UTF-16 BOM 등) 시에만 제한적으로 구동됩니다.
- 일반적인 검색 중 Rust 엔진 내부 오류가 발생하거나 고립 정책에 따라 검색이 불가능할 경우, 상위 시스템은 자동 폴백 대신 해당 파일을 안전하게 스킵 처리합니다.
...
## 5.6.2 변경 사항
- 검색 결과 매치에서 설정된 편집기로 파일과 해당 줄을 열 수 있습니다.
- 시스템 기본 앱을 기본값으로 유지하며 VS Code, Cursor, Notepad++, Sublime Text, 사용자 지정 실행 파일을 선택할 수 있습니다.
- 문맥 미리보기에 JSON, XML, Python, JavaScript, 로그 파일의 경량 구문 강조를 추가했습니다.
- 검색 엔진 경로는 변경하지 않아 일반 검색 성능과 결과 생성 방식에 영향을 주지 않습니다.

## 5.6.1 변경 사항
- 문맥 미리보기 하단에 위·아래 문맥 줄 수 선택 기능을 추가했습니다.
- 위·아래 문맥 표시 범위를 각각 0~20줄로 설정할 수 있으며 기본값은 5줄입니다.
- 문맥 줄 수 설정을 `config.json`에 저장하여 프로그램 재실행 후에도 유지합니다.
- 두 자리 줄 수가 잘리지 않도록 문맥 설정 콤보 박스 표시 폭을 보정했습니다.
- 파일 경로 및 버튼 설명 등 사용자 UI 툴팁을 제거했습니다.
