...
- **극한의 메모리 효율**: 대용량 JSON/Archive 처리 시 **Mmap(Memory-mapped File)** 도입으로 메모리 피크 억제 및 초기 로딩 지연 제거 (v4.63.8)
- 안정성 고도화: `catch_unwind` 기반 파닉 방어 및 워커 타임아웃 자원 회수 메커니즘
- 작업 지속성: 탭 세션, 상태 복원, 파라미터화된 청크 튜닝, 엔진 선택 매트릭스(Docs) 제공
...
- **선별적 폴백**: UTF-16 (BOM), 구조화된 파일(JSON, Archive, Excel) 등 특수 처리가 필요한 경우에만 Python 엔진을 선별 가동합니다.
...
- **Excel/JSON/Archive 무결성**: 구조화된 파일의 형식이 손상되었을 경우(Malformed), 조기 종료하지 않고 반드시 `SKIPPED` 상태로 보고하여 데이터 누락 가능성을 차단합니다. (v4.63.8)
